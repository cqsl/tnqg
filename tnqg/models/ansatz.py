...
class PsiT(nn.Module):
    ...
    def __call__(self, xt):
        ...
        f = self.basis_funcs(t, gammas, self.omegas)
        f0 = self.basis_funcs(jnp.zeros_like(t), gammas, self.omegas)
        c = f - f0

        if self.coeffs_fixed.wrapped.size > 1:
            coeffs_fixed = self.coeffs_fixed.wrapped
            coeffs_fixed = coeffs_fixed.reshape(coeffs_fixed.shape + (1,) * (c[0].ndim))
            coeffs_fixed = coeffs_fixed * jnp.ones_like(c[0])
            coeffs = jnp.concatenate((coeffs_fixed, c), axis=0)
        else:
            coeffs = jnp.concatenate((jnp.ones(c[:1].shape), c), axis=0)

        return jax.scipy.special.logsumexp(logphis, b=coeffs, axis=0)


class Psi0(nn.Module):
    psi0_afun: Any
    logphis_model: Any
    n_models: int
    n_basis: int
    ...
    def __call__(self, x):
        ...
        return jax.scipy.special.logsumexp(logphis[: -self.n_models], b=coeffs, axis=0)
